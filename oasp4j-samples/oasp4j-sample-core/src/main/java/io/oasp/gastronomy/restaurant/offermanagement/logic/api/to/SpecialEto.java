...
public class SpecialEto extends AbstractEto implements Special {
  ...
  // XXX Ich dachte Etos sollen keine anderen Etos enthalten? Ah bei emmbeddable ist das ok
  ...
  public void setActivePeriod(WeeklyPeriod activePeriod) {
    ...
    this.activePeriod = convert(activePeriod);
  }

  private WeeklyPeriodEto convert(WeeklyPeriod wp) {

    WeeklyPeriodEto to = new WeeklyPeriodEto();
    to.setEndingDay(wp.getEndingDay());
    to.setEndingHour(wp.getEndingHour());
    to.setStartingDay(wp.getStartingDay());
    to.setStartingHour(wp.getStartingHour());
    return to;
  }
