...
public class SpecialDaoImpl extends ApplicationDaoImpl<SpecialEntity> implements SpecialDao {
  ...
  private WeeklyPeriodEmbeddable convert(WeeklyPeriodSearchCriteriaTo to) {

    WeeklyPeriodEmbeddable wpe = new WeeklyPeriodEmbeddable();
    wpe.setEndingDay(to.getEndingDay());
    wpe.setEndingHour(to.getEndingHour());
    wpe.setStartingDay(to.getStartingDay());
    wpe.setStartingHour(to.getStartingHour());
    return wpe;
  }
